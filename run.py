...
def get_model(config):
    if config.MODEL == "VGG19":
        ...
        raise Exception("Enter a valid CNN name in config.py")
    return model

def neural_style_transfer(config):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    content_img = images.read_and_resize(config.CONTENT_IMAGE_PTH, new_height= config.RESHAPE_CONTENT_IMAGE, plot=False)
    content_img = images.prepare_img(content_img, device)

    style_img = images.read_and_resize(config.STYLE_IMAGE_PTH, new_height = config.RESHAPE_STYLE_IMAGE, plot=False)
    ...
    for cnt in range(config.EPOCS):
        ...
        with torch.no_grad():
            ...
            images.save_and_maybe_display(input_img, config, cnt, should_display=False)
